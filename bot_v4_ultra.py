#!/usr/bin/env python3
"""
BOT ULTRA v4.0 - NLP + Whale Detection + Multi-Timeframe + Auto-Otimizacao
"""
...
def iniciar_dashboard():
    ...
    log.info(f"🌐 Dashboard rodando na porta {port}")
    ...
    def ajustar_sinal(self, sinal, par, df):
        ...
        if whale:
            ...
            if direcao_whale == sinal:
                ...
                log.info("🐋 Whale CONTRADIZ sinal - aguardando")
...
def score_tf(df):
    ...
    if u["close"] < u["bb_lower"]:
        s += 2
    elif u["close"] > u["bb_upper"]:
        s -= 2
...
def sinal_multi_tf(client, par):
    ...
    log.info(
        f"[{par}] Score M5={scores['M5']['score']:+d} M15={scores['M15']['score']:+d} "
...
class AutoOtimizador:
    ...
    def otimizar(self, client, par="BTCUSDT"):
        ...
        try:
            ...
            log.info(
                f"✅ Otimização: {testados} combinações | "
...
class AgenteQL:
    ...
    def codificar_estado(self, score_mtf, sentimento, nlp_score, whale_ratio):
        s_zona = "FORTE+" if score_mtf >= 7 else (
            "+" if score_mtf >= 4 else (
                "FORTE-" if score_mtf <= -7 else (
                    "-" if score_mtf <= -4 else "N"
                )
            )
        )
        fg_zona = "ME" if sentimento <= 25 else (
            "M" if sentimento <= 45 else (
                "N" if sentimento <= 55 else (
                    "G" if sentimento <= 75 else "GE"
                )
            )
        )
